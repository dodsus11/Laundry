...
Public Class UcRolesAdd
    ...
    Private Sub updateDbRole(ByVal flag As Integer)
        Try
            If Me.tempId = 0 Then 'prosedur insert
                If MsgBox("Anda ingin menyimpan data ini?", MsgBoxStyle.YesNo, "Konfirmasi") = MsgBoxResult.Yes Then
                    ...
                    Dim copyRole As model.Role = Nothing
                    If cmbRole.SelectedValue <> 0 Then
                        copyRole = New model.Role With {.id = cmbRole.SelectedValue}
                    End If
                    userDao.insertRole(role, copyRole)
    ...
    Private Sub UcRolesAdd_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            If Me.tempId = 0 Then ' proses insert role baru
                Dim dt As DataTable = userDao.getAllRoles()
                dt.Rows.Add(0, "Tanpa Salin Role")

                cmbRole.DataSource = dt
                cmbRole.ValueMember = "id"
                cmbRole.DisplayMember = "role_name"
                lblCopyRole.Visible = True
    ...
    Private Sub cmbRole_SelectedValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs)
        Try
